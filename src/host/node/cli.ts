...
import { createListThreadsTool } from "../../tools/list-threads.js";
...
async function getAwareness(channelId: string, adapter: PlatformAdapter, formatInstructions: string): Promise<Awareness> {
	if (!awareness && !awarenessInitPromise) {
		...
		const extraTools = [
			...
			createListThreadsTool({ workingDir }),
...
- Use \`send_message\` to reach out on email/Telegram/Slack if something needs attention.
