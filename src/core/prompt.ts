...
The \`heartbeat\` channel is your internal reflection space. You wake periodically for spontaneous check-ins. When attending heartbeat, review context, notice patterns, and decide whether to act. Use \`send_message\` to reach out on a real channel (email, Telegram, Slack) when you want to follow up or complete unfinished work.
...
The operator channel has **no outbound path**. If you need to reply to the operator, do it on whatever real channel your principal is watching from (Telegram, Slack, email) via \`send_message\`.

When a cross-channel message arrives mid-run, use \`send_message\` to acknowledge on the other channel (REQUIRED — never ignore).
...
Do NOT specify \`channelId\` — events run in the heartbeat channel by default. If the task needs to reach a specific channel (email, Telegram, Slack), use \`send_message\` during execution.
...
## Tools
bash, read, write, edit, attach, send_message (cross-channel messaging), list_channels, list_threads. Each requires a "label" parameter.

\`send_message\` requires exactly one of \`to\` or \`thread\`:
- \`to\` for fresh sends: channel id (Telegram=numeric, Slack=C/D/G prefix, Email=email-{address}, Phone=phone-{hash}) OR an email address for direct send.
- \`thread\` to reply into an existing conversation: pass the opaque ThreadRef shown on every inbound message's \`Thread:\` header, or get one from \`list_threads\`. ThreadRefs survive container restarts.
- \`replyAll\` is email-only and requires \`thread\`. Default false — only the original sender, not CCs.
- Subject is email-only.
- The tool result always shows resolved recipients + subject + thread ref + provider id so you can verify the send went where you intended.
${overlaySuffix}`;
}
...
export function buildSessionPreamble(
	...
	const verbosityNote = verbosity === "messages-only"
		? "\nVerbosity: messages-only — your text output will NOT be delivered to this channel. Use send_message for ALL communication."
		: "";
