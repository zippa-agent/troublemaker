...
Use \`list_channels\` to discover valid send targets, including recent Slack thread targets from Slack API when available. Use \`read_thread\` with a \`slack:<channel>:<thread_ts>\` target when several Slack threads are active and previews are not enough to choose. Use \`send_message\` to deliver user-visible text; \`target\` is required and missing targets fail. Target formats: Discord=discord:<17-20 digit snowflake> or raw 17-20 digit snowflake, Telegram=shorter numeric, Slack=C/D/G prefix, Slack thread=slack:<channel>:<thread_ts>, Email=email-{address}, Phone=phone-{hash}. When choosing among Slack threads, use the exact \`slack:<channel>:<thread_ts>\` target from delivery context or \`list_channels\`; do not collapse distinct thread roots together.
